...
5. **M350 限制**: 無法發送直接馬達 RPM 指令。PSDK 最低層級為 attitude + thrust 或 body-rate。我們的 RL 策略是 DJI 態度控制器上方的 **外迴路控制器**。
...
#### 1A.3 動作空間
```python
# 對應 PSDK Virtual Stick velocity 控制模式
action_space = {
...
psdk/flight_controller.py — Virtual Stick 指令介面
```
